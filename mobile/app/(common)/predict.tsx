...
import { detectKidneyStone } from "@/src/utils/react-query";
import uuid from "react-native-uuid";
import { THistory, TModel } from "@/src/types";

const Page = () => {
  const { camera, gallery, requestCameraPermission } = useMediaPermissions();
  const { model } = useLocalSearchParams<{ model: TModel }>();
  ...
  const { isPending: predicting, mutateAsync } = useMutation({
    mutationKey: ["kidney-prediction"],
    mutationFn: detectKidneyStone,
  });
  ...
  const makeRecommendations = async () => {
    ...
    if (!!!state.uri) {
      setState((s) => ({
        ...s,
        error: "Please select a Kidney image.",
      }));
      return;
    }
    ...
    const image = generateRNFile({
      name: state.fileName || "kidney.jpeg",
      uri: state.uri,
    });
    await mutateAsync({
      image,
      ...
      >
          ...
          <View style={{ flexDirection: "row", flex: 1 }}>
            ...
            >
              <Text style={{ fontFamily: FONTS.bold, fontSize: 25 }}>
                Kidney Image Selection
              </Text>
              <Text style={{ fontFamily: FONTS.regular, color: COLORS.gray }}>
                Select a kidney image or take it live from camera.
              </Text>
            </View>
          </View>
        ...
        >
          <View style={{ flexDirection: "row", flex: 1 }}>
            ...
            >
              <Text style={{ fontFamily: FONTS.bold, fontSize: 25 }}>
                Then, Does This Kidney Have Stones?
              </Text>
              <Text style={{ fontFamily: FONTS.regular, color: COLORS.gray }}>
